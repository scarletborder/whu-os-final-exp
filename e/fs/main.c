...
PRIVATE void mkfs() {
	...
	printl(
		...
		geo.base * 2, (geo.base + 1) * 2, (geo.base + 1 + 1) * 2, (geo.base + 1 + 1 + sb.nr_imap_sects) * 2, (geo.base + 1 + 1 + sb.nr_imap_sects + sb.nr_smap_sects) * 2, (geo.base + sb.n_1st_sect) * 2);
...
/**
 * similar with `get_inode()`
 *
 * only use this when seek inode in the inode link
 */
PUBLIC struct inode *get_inode_directly(int dev, int num) {
	if (num == 0)
		return 0;
...
/**
 * count_surplus_inode_link
 *
 * add cnt of the left inodes in the inode link (except for the first inode)
 * usually be called when call `get_inode`
 *
 * @note DO NOT USE WITH function which will also add count to surplus inodes
 * like call `get_inode()` to the left inodes in the link.
 *
 * @param firstInode the first inode of the link
 */
PUBLIC void count_surplus_inode_link(struct inode *firstInode) {
	while (firstInode->i_next_node != INVALID_INODE) {
		firstInode = get_inode_directly(firstInode->i_dev, firstInode->i_next_node);
		firstInode->i_cnt++;
	}
}
 ...
 * a new inode.
 *
 * @param pinode first I-node ptr.
 *****************************************************************************/
PUBLIC void put_inodes_link(struct inode *pinode) {
	if (pinode->i_next_node == 0) {
		put_inode(pinode);
		return;
	} else {
		put_inodes_link(get_inode_directly(pinode->i_dev, pinode->i_next_node));
		put_inode(pinode);
	}
}
 ...
 *****************************************************************************/
/**
 * <Ring 1> Write a single inode back to the disk. Commonly invoked as soon
 * 			as the inode is changed.
 *
 * @param p I-node ptr.
 *****************************************************************************/
 ...
 *****************************************************************************/
/**
 * <Ring 1> Write the inodes link back to the disk. Commonly invoked as soon
 * 			as the inode is changed.
 *
 * @param p first I-node ptr.
 *****************************************************************************/
PUBLIC void sync_inode_link(struct inode *p) {
	if (p->i_next_node == 0) {
		sync_inode(p);
		return;
	} else {
		sync_inode_link(get_inode_directly(p->i_dev, p->i_next_node));
		sync_inode(p);
	}
}
