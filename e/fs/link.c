...
PUBLIC int do_unlink()
{
	...
	struct inode * dir_inode; // 获取 dir's inode
	...
	struct inode* first_inode = pin;
	while (1){
		int next_pin_nr = pin->i_next_node;
		int pin_dev = pin->i_dev;
		...
		*/
		...
		if (next_pin_nr == INVALID_INODE){
			break;
		}
		pin = get_inode_directly(pin_dev, next_pin_nr);
	}
