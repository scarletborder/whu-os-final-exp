...
PUBLIC int do_lseek()
{
	...
	struct inode* traverse_node = pcaller->filp[fd]->fd_inode;
	int traverse_node_nr = traverse_node->i_num;

	u64 f_size = 0;

	do{
		f_size += traverse_node->i_size;
		traverse_node_nr = traverse_node->i_next_node;
		if (traverse_node_nr != INVALID_INODE) {
			break;
		}
		traverse_node = get_inode_directly(traverse_node->i_dev, traverse_node_nr);
	} while (traverse_node_nr != INVALID_INODE || traverse_node);
...
PRIVATE struct inode * new_inode_custom(int dev, int inode_nr, u32 imode,int start_sect, u32 nr_sects) {
	...
	new_inode->i_next_node = INVALID_INODE;
